...
/**
 * This program will read words from an input file, and count the number of
 * occurrences of each word. The word data is written to an output file twice,
 * once with the words in alphabetical order and once with the words ordered by
 * number of occurrences. The user specifies the input file and the output file.
 *
 * The program demonstrates several parts of Java's framework for generic
 * programming: TreeMap, List sorting, Comparators, etc.
 */
public class WordCount {
    ...
    /**
     * A comparator for comparing objects of type WordData according to their
     * counts. This is used for sorting the list of words by frequency.
     */
    ...
    public static ArrayList<WordData> ProcessFile(String path) throws FileNotFoundException
    {
        try{
        File selectedFile = new File(path);
        TextIO.in= new BufferedReader( new FileReader(selectedFile));
        ...
        TextIO.inputErrorCount = 0;
         ...
         TreeMap<String, WordData> words = new TreeMap<String, WordData>();
            ...
            while (word != null) {
                ...
                /* WARNING!!!*/
                //if (!Arrays.asList(NotMatter.nowords).contains(word)) {
            ...
            for (Object key : new ArrayList<Object>(words.keySet())) {
                if (Arrays.asList(NotMatter.nowords).contains(key)) {
                    words.remove(key);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        //System.in.readln();  // Wait for user to press return.
...
//            }
        ...
        System.exit(
                0);  // Might be necessary, because of use of file dialogs.
    }
